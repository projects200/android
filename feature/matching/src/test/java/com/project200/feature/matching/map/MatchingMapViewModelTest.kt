...
class MatchingMapViewModelTest {
    ...
    private val sampleExercisePlaces =
        listOf(
    ...
    private val sampleMatchingMembers =
        listOf(
            MatchingMember(
                memberId = "member1",
    ...
    @Test
    fun `init - 최초 방문 시 가이드 이벤트가 emit된다`() =
        runTest {
    ...
    @Test
    fun `init - 운동 장소 없을 시 다이얼로그 이벤트가 emit된다`() =
        runTest {
            coEvery { mockGetLastMapPositionUseCase() } returns sampleMapPosition
    ...
    @Test
    fun `refreshExercisePlaces - 운동 장소가 새로고침된다`() =
        runTest {
            coEvery { mockGetLastMapPositionUseCase() } returns sampleMapPosition
            coEvery { mockGetExercisePlaceUseCase() } returns BaseResult.Success(sampleExercisePlaces)
            coEvery { mockGetExerciseTypesUseCase() } returns BaseResult.Success(sampleExerciseTypes)
    ...
    @Test
    fun `onFilterTypeClicked - 필터 타입이 emit된다`() =
        runTest {
            coEvery { mockGetLastMapPositionUseCase() } returns sampleMapPosition
            coEvery { mockGetExercisePlaceUseCase() } returns BaseResult.Success(sampleExercisePlaces)
            coEvery { mockGetExerciseTypesUseCase() } returns BaseResult.Success(sampleExerciseTypes)
    ...
    @Test
    fun `onFilterOptionSelected - DAY 필터가 토글된다`() =
        runTest {
            coEvery { mockGetLastMapPositionUseCase() } returns sampleMapPosition
            coEvery { mockGetExercisePlaceUseCase() } returns BaseResult.Success(sampleExercisePlaces)
            coEvery { mockGetExerciseTypesUseCase() } returns BaseResult.Success(sampleExerciseTypes)
    ...
    @Test
    fun `onFilterOptionSelected - EXERCISE_TYPE 필터가 설정된다`() =
        runTest {
            coEvery { mockGetLastMapPositionUseCase() } returns sampleMapPosition
            coEvery { mockGetExercisePlaceUseCase() } returns BaseResult.Success(sampleExercisePlaces)
            coEvery { mockGetExerciseTypesUseCase() } returns BaseResult.Success(sampleExerciseTypes)
    ...
    @Test
    fun `fetchMatchingMembersIfMoved - 줌 레벨 낮으면 회원 목록 비우고 경고 emit`() =
        runTest {
            coEvery { mockGetLastMapPositionUseCase() } returns sampleMapPosition
            coEvery { mockGetExercisePlaceUseCase() } returns BaseResult.Success(sampleExercisePlaces)
            coEvery { mockGetExerciseTypesUseCase() } returns BaseResult.Success(sampleExerciseTypes)
    ...
    @Test
    fun `fetchMatchingMembersIfMoved - 충분히 이동 시 회원 목록 조회`() =
        runTest {
            coEvery { mockGetLastMapPositionUseCase() } returns sampleMapPosition
            coEvery { mockGetExercisePlaceUseCase() } returns BaseResult.Success(sampleExercisePlaces)
            coEvery { mockGetExerciseTypesUseCase() } returns BaseResult.Success(sampleExerciseTypes)
    ...
    @Test
    fun `fetchMatchingMembersIfMoved - 작은 이동은 조회하지 않음`() =
        runTest {
            coEvery { mockGetLastMapPositionUseCase() } returns sampleMapPosition
            coEvery { mockGetExercisePlaceUseCase() } returns BaseResult.Success(sampleExercisePlaces)
            coEvery { mockGetExerciseTypesUseCase() } returns BaseResult.Success(sampleExerciseTypes)
