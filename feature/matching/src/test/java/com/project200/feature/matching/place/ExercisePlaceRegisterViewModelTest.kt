...
class ExercisePlaceRegisterViewModelTest {
    ...
    @Test
    fun `initializePlaceInfo - 장소 정보가 초기화된다`() = runTest {
    ...
    @Test
    fun `onPlaceNameChanged - 장소명이 업데이트된다`() = runTest {
        viewModel = createViewModel()
    ...
    @Test
    fun `confirmExercisePlace - 장소명이 비어있으면 아무것도 하지 않는다`() = runTest {
        viewModel = createViewModel()
    ...
    @Test
    fun `confirmExercisePlace - 새 장소(id=-1)인 경우 registerUseCase가 호출된다`() = runTest {
    ...
    @Test
    fun `confirmExercisePlace - 기존 장소(id!=-1)인 경우 editUseCase가 호출된다`() = runTest {
    ...
    @Test
    fun `confirmExercisePlace - 등록 성공 시 registrationResult가 업데이트된다`() = runTest {
        coEvery { mockRegisterExercisePlaceUseCase(any()) } returns BaseResult.Success(Unit)
    ...
    @Test
    fun `confirmExercisePlace - 수정 성공 시 editResult가 업데이트된다`() = runTest {
        coEvery { mockEditExercisePlaceUseCase(any()) } returns BaseResult.Success(Unit)
    ...
    @Test
    fun `confirmExercisePlace - 등록 실패 시 registrationResult에 Error가 설정된다`() = runTest {
    ...
    @Test
    fun `confirmExercisePlace - 변경된 이름으로 장소가 등록된다`() = runTest {
        coEvery { mockRegisterExercisePlaceUseCase(any()) } returns BaseResult.Success(Unit)
