...
class ExercisePlaceSearchViewModelTest {
    ...
    @Test
    fun `fetchAddressFromCoordinates - 성공 시 place가 업데이트된다`() = runTest {
    ...
    @Test
    fun `fetchAddressFromCoordinates - 성공 시 placeInfoResult가 Success로 업데이트된다`() = runTest {
        coEvery { mockGetAddressFromCoordinatesUseCase(any(), any()) } returns BaseResult.Success(samplePlaceInfo)
    ...
    @Test
    fun `fetchAddressFromCoordinates - 실패 시 place는 업데이트되지 않고 placeInfoResult만 Error로 설정된다`() = runTest {
    ...
    @Test
    fun `searchPlacesByKeyword - 검색 결과가 searchResult에 업데이트된다`() = runTest {
    ...
    @Test
    fun `selectSearchedPlace - placeName이 비어있으면 빈 문자열로 설정된다`() = runTest {
    ...
    @Test
    fun `selectSearchedPlace - placeName이 address와 같으면 빈 문자열로 설정된다`() = runTest {
        val placeWithSameNameAndAddress = KakaoPlaceInfo(
            placeName = "서울시 강남구",
            address = "서울시 강남구",
            latitude = 37.5,
            longitude = 127.0
    ...
    @Test
    fun `selectSearchedPlace - 유효한 placeName이면 그대로 설정된다`() = runTest {
        viewModel = createViewModel()
    ...
    @Test
    fun `onMapMoved - selectSearchedPlace 직후에는 API를 호출하지 않는다`() = runTest {
        coEvery { mockGetAddressFromCoordinatesUseCase(any(), any()) } returns BaseResult.Success(samplePlaceInfo)
    ...
    @Test
    fun `onMapMoved - 선택 없이 이동하면 fetchAddressFromCoordinates가 호출된다`() = runTest {
        coEvery { mockGetAddressFromCoordinatesUseCase(any(), any()) } returns BaseResult.Success(samplePlaceInfo)
    ...
    @Test
    fun `onMapMoved - selectSearchedPlace 후 두번째 이동부터는 API가 호출된다`() = runTest {
        coEvery { mockGetAddressFromCoordinatesUseCase(any(), any()) } returns BaseResult.Success(samplePlaceInfo)
