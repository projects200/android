...
class MatchingProfileViewModelTest {
    ...
    @Test
    fun `setInitialData - memberId와 placeId가 설정되고 프로필이 로드된다`() =
        runTest {
    ...
    @Test
    fun `getProfile - 성공 시 profile이 업데이트된다`() =
        runTest {
            coEvery { mockGetMatchingProfileUseCase(any()) } returns BaseResult.Success(sampleProfile)
            coEvery { mockGetMemberExerciseUseCase(any(), any(), any()) } returns BaseResult.Success(sampleExerciseCounts)
    ...
    @Test
    fun `onMonthChanged - 새로운 월은 getMemberExerciseUseCase를 호출한다`() =
        runTest {
            coEvery { mockGetMatchingProfileUseCase(any()) } returns BaseResult.Success(sampleProfile)
            coEvery { mockGetMemberExerciseUseCase(any(), any(), any()) } returns BaseResult.Success(sampleExerciseCounts)
    ...
    @Test
    fun `onPreviousMonthClicked - 이전 월로 변경된다`() =
        runTest {
            coEvery { mockGetMatchingProfileUseCase(any()) } returns BaseResult.Success(sampleProfile)
            coEvery { mockGetMemberExerciseUseCase(any(), any(), any()) } returns BaseResult.Success(sampleExerciseCounts)
    ...
    @Test
    fun `onNextMonthClicked - 미래가 아닌 경우 다음 월로 변경된다`() =
        runTest {
            coEvery { mockGetMatchingProfileUseCase(any()) } returns BaseResult.Success(sampleProfile)
            coEvery { mockGetMemberExerciseUseCase(any(), any(), any()) } returns BaseResult.Success(sampleExerciseCounts)
    ...
    @Test
    fun `onNextMonthClicked - 미래인 경우 변경되지 않는다`() =
        runTest {
            coEvery { mockGetMatchingProfileUseCase(any()) } returns BaseResult.Success(sampleProfile)
            coEvery { mockGetMemberExerciseUseCase(any(), any(), any()) } returns BaseResult.Success(sampleExerciseCounts)
    ...
    @Test
    fun `blockMember - 성공 시 blockResult가 emit된다`() =
        runTest {
            coEvery { mockGetMatchingProfileUseCase(any()) } returns BaseResult.Success(sampleProfile)
            coEvery { mockGetMemberExerciseUseCase(any(), any(), any()) } returns BaseResult.Success(sampleExerciseCounts)
    ...
    @Test
    fun `blockMember - 실패 시 toast가 emit된다`() =
        runTest {
            coEvery { mockGetMatchingProfileUseCase(any()) } returns BaseResult.Success(sampleProfile)
            coEvery { mockGetMemberExerciseUseCase(any(), any(), any()) } returns BaseResult.Success(sampleExerciseCounts)
    ...
    @Test
    fun `exerciseDates - 운동 기록이 있는 날짜가 업데이트된다`() =
        runTest {
            coEvery { mockGetMatchingProfileUseCase(any()) } returns BaseResult.Success(sampleProfile)
            coEvery { mockGetMemberExerciseUseCase(any(), any(), any()) } returns BaseResult.Success(sampleExerciseCounts)
