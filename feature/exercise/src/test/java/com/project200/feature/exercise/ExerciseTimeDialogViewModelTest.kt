...
class ExerciseTimeDialogViewModelTest {
    ...
    @Test
    fun `onDateTimeConfirmed - 선택된 시간이 오늘 미래 시간일 경우, ShowFutureTimeErrorToast를 발행한다`() =
        runTest {
    ...
    @Test
    fun `onDateTimeConfirmed - 선택된 시간이 오늘 과거 시간일 경우, TimeSelected를 발행한다`() =
        runTest {
            // Given: 현재 시간이 2025년 6월 27일 오전 10시 0분으로 가정
            val currentMockDateTime = LocalDateTime.of(2025, 6, 27, 10, 0)
            every { mockClockProvider.localDateTimeNow() } returns currentMockDateTime // ClockProvider의 localDateTimeNow() 사용
    ...
    @Test
    fun `onDateTimeConfirmed - 선택된 시간이 오늘 현재 시간일 경우, TimeSelected를 발행한다`() =
        runTest {
            // Given: 현재 시간이 2025년 6월 27일 오전 10시 0분으로 가정
            val currentMockDateTime = LocalDateTime.of(2025, 6, 27, 10, 0)
            every { mockClockProvider.localDateTimeNow() } returns currentMockDateTime // ClockProvider의 localDateTimeNow() 사용
    ...
    @Test
    fun `onDateTimeConfirmed - 선택된 날짜가 과거일 경우, TimeSelected를 발행한다 (시간 무관)`() =
        runTest {
            // Given: 현재 시간이 2025년 6월 27일 오전 10시 0분으로 가정
            val currentMockDateTime = LocalDateTime.of(2025, 6, 27, 10, 0)
            every { mockClockProvider.localDateTimeNow() } returns currentMockDateTime // ClockProvider의 localDateTimeNow() 사용
