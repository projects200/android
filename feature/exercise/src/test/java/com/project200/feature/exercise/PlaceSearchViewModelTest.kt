...
class PlaceSearchViewModelTest {
    ...
    @Test
    fun `fetchAddressFromCoordinates - 성공하면 place와 placeInfoResult가 업데이트된다`() =
        runTest {
            // Given
    ...
    @Test
    fun `fetchAddressFromCoordinates - 성공하면 좌표가 정확히 설정된다`() =
        runTest {
            // Given
    ...
    @Test
    fun `fetchAddressFromCoordinates - 실패하면 placeInfoResult에 에러가 설정된다`() =
        runTest {
            // Given
    ...
    @Test
    fun `searchPlacesByKeyword - 성공하면 검색 결과가 반환된다`() =
        runTest {
            // Given
    ...
    @Test
    fun `searchPlacesByKeyword - 실패하면 에러가 반환된다`() =
        runTest {
            // Given
    ...
    @Test
    fun `selectSearchedPlace - 장소 선택 시 place가 업데이트된다`() =
        runTest {
            // Given
            viewModel = createViewModel()
    ...
    @Test
    fun `selectSearchedPlace - placeName이 비어있으면 그대로 설정된다`() =
        runTest {
            // Given
            viewModel = createViewModel()
    ...
    @Test
    fun `selectSearchedPlace - placeName이 address와 같으면 placeName을 빈 문자열로 설정한다`() =
        runTest {
            // Given
            viewModel = createViewModel()
            val placeWithSameName =
                KakaoPlaceInfo(
                    placeName = "서울시 강남구",
                    address = "서울시 강남구",
                    latitude = 37.5,
    ...
    @Test
    fun `onMapMoved - 검색으로 선택한 직후에는 주소 조회를 하지 않는다`() =
        runTest {
            // Given
            coEvery { mockGetAddressFromCoordinatesUseCase(any(), any()) } returns BaseResult.Success(samplePlaceInfo)
    ...
    @Test
    fun `onMapMoved - 일반 지도 이동 시 주소 조회를 수행한다`() =
        runTest {
            // Given
            coEvery { mockGetAddressFromCoordinatesUseCase(any(), any()) } returns BaseResult.Success(samplePlaceInfo)
    ...
    @Test
    fun `onMapMoved - 검색 선택 후 두 번째 이동부터는 주소 조회를 수행한다`() =
        runTest {
            // Given
            coEvery { mockGetAddressFromCoordinatesUseCase(any(), any()) } returns BaseResult.Success(samplePlaceInfo)
