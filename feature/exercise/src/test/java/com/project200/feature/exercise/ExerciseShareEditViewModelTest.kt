...
class ExerciseShareEditViewModelTest {
    ...
    private val sampleRecordNoPictures =
        ExerciseRecord(
            title = "오늘의 운동",
            detail = "하체 운동 완료",
            personalType = "STRENGTH",
    ...
    private val sampleTransformInfo =
        StickerTransformInfo(
    ...
    @Test
    fun `loadExerciseRecord - 성공하면 운동 기록이 로드된다`() =
        runTest {
            // Given
    ...
    @Test
    fun `loadExerciseRecord - 성공하면 첫 번째 사진이 배경 이미지로 설정된다`() =
        runTest {
            // Given
            coEvery { mockGetExerciseRecordDetailUseCase(any()) } returns BaseResult.Success(sampleRecord)
    ...
    @Test
    fun `loadExerciseRecord - 사진이 없으면 배경 이미지는 null이다`() =
        runTest {
            // Given
    ...
    @Test
    fun `loadExerciseRecord - 실패해도 로딩이 완료된다`() =
        runTest {
            // Given
    ...
    @Test
    fun `loadExerciseRecord - 올바른 recordId로 UseCase가 호출된다`() =
        runTest {
            // Given
            coEvery { mockGetExerciseRecordDetailUseCase(any()) } returns BaseResult.Success(sampleRecord)
    ...
    @Test
    fun `selectTheme - 모든 테마로 변경 가능하다`() =
        runTest {
            // Given
            viewModel = createViewModel()
    ...
    @Test
    fun `requestShare - 운동 기록이 있으면 shareEvent가 발생한다`() =
        runTest {
            // Given
            coEvery { mockGetExerciseRecordDetailUseCase(any()) } returns BaseResult.Success(sampleRecord)
    ...
    @Test
    fun `requestShare - 운동 기록이 없으면 shareEvent가 발생하지 않는다`() =
        runTest {
            // Given
            viewModel = createViewModel()
    ...
    @Test
    fun `requestShare - 선택된 테마가 이벤트에 포함된다`() =
        runTest {
            // Given
            coEvery { mockGetExerciseRecordDetailUseCase(any()) } returns BaseResult.Success(sampleRecord)
    ...
    @Test
    fun `requestShare - 로딩 상태가 true로 설정된다`() =
        runTest {
            // Given
            coEvery { mockGetExerciseRecordDetailUseCase(any()) } returns BaseResult.Success(sampleRecord)
    ...
    @Test
    fun `onShareCompleted - 로딩 상태가 false로 설정된다`() =
        runTest {
            // Given
            coEvery { mockGetExerciseRecordDetailUseCase(any()) } returns BaseResult.Success(sampleRecord)
    ...
    @Test
    fun `stickerState - 운동 기록과 테마가 결합된다`() =
        runTest {
            // Given
            coEvery { mockGetExerciseRecordDetailUseCase(any()) } returns BaseResult.Success(sampleRecord)
    ...
    @Test
    fun `stickerState - 테마 변경 시 새로운 상태가 emit된다`() =
        runTest {
            // Given
            coEvery { mockGetExerciseRecordDetailUseCase(any()) } returns BaseResult.Success(sampleRecord)
