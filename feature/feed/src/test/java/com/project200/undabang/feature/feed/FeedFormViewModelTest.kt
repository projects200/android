...
class FeedFormViewModelTest {
    ...
    @Test
    fun `initData - 생성 모드로 초기화하면 isEditMode가 false`() =
        runTest {
            // Given
    ...
    @Test
    fun `initData - 수정 모드로 초기화하면 isEditMode가 true`() =
        runTest {
            // Given
            coEvery { getUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
            coEvery { getPreferredExerciseUseCase() } returns BaseResult.Success(listOf(sampleExercise))
    ...
    @Test
    fun `initData - 수정 모드에서 기존 내용을 로드한다`() =
        runTest {
            // Given
            coEvery { getUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
            coEvery { getPreferredExerciseUseCase() } returns BaseResult.Success(listOf(sampleExercise))
    ...
    @Test
    fun `initData - 프로필 로드 실패 시 토스트 이벤트 발생`() =
        runTest {
            // Given
    ...
    @Test
    fun `selectType - 운동 타입을 선택할 수 있다`() =
        runTest {
            // Given
            coEvery { getUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
            coEvery { getPreferredExerciseUseCase() } returns BaseResult.Success(listOf(sampleExercise))
    ...
    @Test
    fun `submitFeed - 빈 내용은 토스트를 표시한다`() =
        runTest {
            // When & Then
            viewModel.toastEvent.test {
    ...
    @Test
    fun `submitFeed - 생성 모드에서 피드 생성 성공`() =
        runTest {
            // Given
            coEvery { getUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
            coEvery { getPreferredExerciseUseCase() } returns BaseResult.Success(listOf(sampleExercise))
    ...
    @Test
    fun `submitFeed - 생성 모드에서 이미지와 함께 피드 생성`() =
        runTest {
            // Given
            coEvery { getUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
            coEvery { getPreferredExerciseUseCase() } returns BaseResult.Success(emptyList())
    ...
    @Test
    fun `submitFeed - 수정 모드에서 피드 업데이트 성공`() =
        runTest {
            // Given
            coEvery { getUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
            coEvery { getPreferredExerciseUseCase() } returns BaseResult.Success(listOf(sampleExercise))
    ...
    @Test
    fun `submitFeed - 생성 실패 시 토스트 이벤트 발생`() =
        runTest {
            // Given
            coEvery { getUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
            coEvery { getPreferredExerciseUseCase() } returns BaseResult.Success(emptyList())
    ...
    @Test
    fun `requestShowDabangSelection - 운동 타입 목록이 있으면 바텀시트를 표시한다`() =
        runTest {
            // Given
            coEvery { getUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
            coEvery { getPreferredExerciseUseCase() } returns BaseResult.Success(listOf(sampleExercise))
