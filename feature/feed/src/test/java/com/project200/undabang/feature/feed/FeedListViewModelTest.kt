...
class FeedListViewModelTest {
    ...
    private val samplePreferredExercise = PreferredExercise(
    ...
    @Test
    fun `init - ViewModel 생성 시 피드 목록을 로드한다`() = runTest {
        // When
    ...
    @Test
    fun `init - ViewModel 생성 시 운동 타입을 로드한다`() = runTest {
        // When
        viewModel = createViewModel()
        testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `init - ViewModel 생성 시 현재 회원 ID를 로드한다`() = runTest {
        // When
        viewModel = createViewModel()
        testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `selectType - 타입 선택 시 필터가 적용된다`() = runTest {
        // Given
        viewModel = createViewModel()
        testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `clearType - 타입 초기화 시 필터가 해제된다`() = runTest {
        // Given
        viewModel = createViewModel()
        testDispatcher.scheduler.advanceUntilIdle()
        viewModel.selectType("헬스")
    ...
    @Test
    fun `canLoadMore - 타입 필터가 적용되면 false`() = runTest {
        // Given
        viewModel = createViewModel()
        testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `loadFeeds - 새로고침 시 목록을 초기화하고 다시 로드한다`() = runTest {
        // Given
        viewModel = createViewModel()
        testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `loadFeeds - 에러 발생 시 토스트 이벤트를 발생시킨다`() = runTest {
        // Given
    ...
    @Test
    fun `deleteFeed - 삭제 성공 시 목록에서 제거된다`() = runTest {
        // Given
        viewModel = createViewModel()
        testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `requestShowCategoryBottomSheet - 운동 타입 목록이 있으면 바텀시트를 표시한다`() = runTest {
        // Given
        viewModel = createViewModel()
        testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `onCategoryBottomSheetShown - 바텀시트 표시 후 상태를 초기화한다`() = runTest {
        // Given
        viewModel = createViewModel()
        testDispatcher.scheduler.advanceUntilIdle()
        viewModel.requestShowCategoryBottomSheet()
