...
class MypageViewModelTest {
    ...
    private val sampleExerciseCounts =
        listOf(
    ...
    @Test
    fun `init - ViewModel 초기화 시 getProfile과 onMonthChanged가 호출된다`() =
        runTest {
            // Given
    ...
    @Test
    fun `getProfile - 성공 시 profile LiveData가 업데이트된다`() =
        runTest {
            // Given
            coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
            coEvery { mockGetExerciseCountInMonthUseCase(any(), any()) } returns BaseResult.Success(sampleExerciseCounts)
    ...
    @Test
    fun `getProfile - 실패 시 toast가 emit된다`() =
        runTest {
            // Given
    ...
    @Test
    fun `onMonthChanged - 캐시된 월은 API를 호출하지 않는다`() =
        runTest {
            // Given
            coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
            coEvery { mockGetExerciseCountInMonthUseCase(any(), any()) } returns BaseResult.Success(sampleExerciseCounts)
    ...
    @Test
    fun `onMonthChanged - 새로운 월은 getExerciseCounts를 호출한다`() =
        runTest {
            // Given
            coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
            coEvery { mockGetExerciseCountInMonthUseCase(any(), any()) } returns BaseResult.Success(sampleExerciseCounts)
    ...
    @Test
    fun `onMonthChanged - 운동 기록 조회 성공 시 exerciseDates가 업데이트된다`() =
        runTest {
            // Given
            coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
            coEvery { mockGetExerciseCountInMonthUseCase(any(), any()) } returns BaseResult.Success(sampleExerciseCounts)
    ...
    @Test
    fun `onMonthChanged - 운동 기록 조회 실패 시 toast가 emit된다`() =
        runTest {
            // Given
            coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
    ...
    @Test
    fun `onPreviousMonthClicked - 이전 월로 selectedMonth가 변경된다`() =
        runTest {
            // Given
            coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
            coEvery { mockGetExerciseCountInMonthUseCase(any(), any()) } returns BaseResult.Success(sampleExerciseCounts)
    ...
    @Test
    fun `onNextMonthClicked - 다음 월이 현재 월보다 이전이면 selectedMonth가 변경된다`() =
        runTest {
            // Given
            coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
            coEvery { mockGetExerciseCountInMonthUseCase(any(), any()) } returns BaseResult.Success(sampleExerciseCounts)
    ...
    @Test
    fun `onNextMonthClicked - 다음 월이 미래이면 selectedMonth가 변경되지 않는다`() =
        runTest {
            // Given
            coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
            coEvery { mockGetExerciseCountInMonthUseCase(any(), any()) } returns BaseResult.Success(sampleExerciseCounts)
    ...
    @Test
    fun `getProfile - 수동 호출 시 프로필이 다시 로드된다`() =
        runTest {
            // Given
            coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
            coEvery { mockGetExerciseCountInMonthUseCase(any(), any()) } returns BaseResult.Success(sampleExerciseCounts)
