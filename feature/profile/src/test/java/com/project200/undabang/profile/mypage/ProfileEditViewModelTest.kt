...
class ProfileEditViewModelTest {
    ...
    @Test
    fun `init - ViewModel 초기화 시 getProfile이 호출된다`() = runTest {
        // Given
    ...
    @Test
    fun `getProfile - 성공 시 initProfile이 업데이트된다`() = runTest {
        // Given
        coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
    ...
    @Test
    fun `getProfile - 실패 시 LOAD_FAILED 에러가 emit된다`() = runTest {
        // Given
    ...
    @Test
    fun `updateNickname - 닉네임 변경 시 isNicknameChecked가 false로 리셋된다`() = runTest {
        // Given
        coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
        createViewModel()
        testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `updateNickname - 동일한 닉네임으로 변경 시 상태가 변하지 않는다`() = runTest {
        // Given
        coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
        createViewModel()
        testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `selectGender - 성별 선택 시 gender가 업데이트된다`() = runTest {
        // Given
        coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
        createViewModel()
        testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `updateProfileImageUri - 프로필 이미지 URI가 업데이트된다`() = runTest {
        // Given
        coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
        createViewModel()
        testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `checkIsNicknameDuplicated - 기존 닉네임과 동일하면 SAME_AS_ORIGINAL 에러가 emit된다`() = runTest {
        // Given
        coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
        createViewModel()
        testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `checkIsNicknameDuplicated - 닉네임 유효성 검사 실패 시 INVALID 상태가 설정된다`() = runTest {
        // Given
        coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
    ...
    @Test
    fun `checkIsNicknameDuplicated - 사용 가능한 닉네임이면 AVAILABLE 상태가 설정된다`() = runTest {
        // Given
        coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
    ...
    @Test
    fun `checkIsNicknameDuplicated - 중복된 닉네임이면 DUPLICATED 상태가 설정된다`() = runTest {
        // Given
        coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
        every { mockValidateNicknameUseCase(any()) } returns true
    ...
    @Test
    fun `checkIsNicknameDuplicated - API 에러 시 CHECK_DUPLICATE_FAILED 에러가 emit된다`() = runTest {
        // Given
        coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
        every { mockValidateNicknameUseCase(any()) } returns true
    ...
    @Test
    fun `completeEditProfile - 변경사항 없으면 NO_CHANGE 에러가 emit된다`() = runTest {
        // Given
        coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
        createViewModel()
        testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `completeEditProfile - 닉네임 변경 시 중복확인 안하면 NO_DUPLICATE_CHECKED 에러가 emit된다`() = runTest {
        // Given
        coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
        createViewModel()
        testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `completeEditProfile - 이미지와 프로필 모두 변경 성공 시 true가 emit된다`() = runTest {
        // Given
        coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
        every { mockValidateNicknameUseCase(any()) } returns true
        coEvery { mockCheckNicknameDuplicatedUseCase(any()) } returns BaseResult.Success(true)
    ...
    @Test
    fun `completeEditProfile - 이미지만 변경 시 editProfileUseCase는 호출되지 않는다`() = runTest {
        // Given
        coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
        coEvery { mockAddProfileImageUseCase(any()) } returns BaseResult.Success(Unit)
        createViewModel()
    ...
    @Test
    fun `completeEditProfile - 이미지 업로드 실패 시 false가 emit된다`() = runTest {
        // Given
        coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
    ...
    @Test
    fun `postImageError - 이미지 에러가 emit된다`() = runTest {
        // Given
        coEvery { mockGetUserProfileUseCase() } returns BaseResult.Success(sampleProfile)
        createViewModel()
        testDispatcher.scheduler.advanceUntilIdle()
