...
class ProfileImageDetailViewModelTest {
    ...
    private val sampleImageList = ProfileImageList(
    ...
    private fun createViewModel() {
        viewModel = ProfileImageDetailViewModel(
    ...
    @Test
    fun `init - ViewModel 초기화 시 getProfileImageList가 호출된다`() = runTest {
        // Given
    ...
    @Test
    fun `getProfileImageList - 성공 시 썸네일이 맨 앞에 위치한 리스트가 반환된다`() = runTest {
        // Given
        coEvery { mockGetProfileImagesUseCase() } returns BaseResult.Success(sampleImageList)
    ...
    @Test
    fun `getProfileImageList - 썸네일 없이 이미지만 있는 경우 이미지 리스트가 반환된다`() = runTest {
        // Given
    ...
    @Test
    fun `getProfileImageList - 이미지가 없는 경우 더미 이미지가 추가된다`() = runTest {
        // Given
    ...
    @Test
    fun `getProfileImageList - 실패 시 LOAD_FAILED 에러가 emit된다`() = runTest {
        // Given
    ...
    @Test
    fun `deleteImage - 성공 시 이미지가 리스트에서 제거된다`() = runTest {
        // Given
        coEvery { mockGetProfileImagesUseCase() } returns BaseResult.Success(sampleImageList)
    ...
    @Test
    fun `deleteImage - 성공 후 리스트가 비면 더미 이미지가 추가된다`() = runTest {
        // Given
        val singleImageList = ProfileImageList(
            thumbnail = sampleThumbnail,
            images = emptyList()
        )
    ...
    @Test
    fun `deleteImage - 성공 시 imageDeleteResult에 Success가 emit된다`() = runTest {
        // Given
        coEvery { mockGetProfileImagesUseCase() } returns BaseResult.Success(sampleImageList)
        coEvery { mockDeleteProfileImageUseCase(any()) } returns BaseResult.Success(Unit)
        createViewModel()
    ...
    @Test
    fun `deleteImage - 실패 시 imageDeleteResult에 Error가 emit된다`() = runTest {
        // Given
        coEvery { mockGetProfileImagesUseCase() } returns BaseResult.Success(sampleImageList)
    ...
    @Test
    fun `changeThumbnail - 성공 시 새 썸네일이 리스트 맨 앞으로 이동한다`() = runTest {
        // Given
        coEvery { mockGetProfileImagesUseCase() } returns BaseResult.Success(sampleImageList)
    ...
    @Test
    fun `changeThumbnail - 성공 시 changeThumbnailResult에 Success가 emit된다`() = runTest {
        // Given
        coEvery { mockGetProfileImagesUseCase() } returns BaseResult.Success(sampleImageList)
        coEvery { mockChangeThumbnailUseCase(any()) } returns BaseResult.Success(Unit)
        createViewModel()
    ...
    @Test
    fun `changeThumbnail - 실패 시 리스트가 변경되지 않고 Error가 emit된다`() = runTest {
        // Given
        coEvery { mockGetProfileImagesUseCase() } returns BaseResult.Success(sampleImageList)
    ...
    @Test
    fun `getProfileImageList - 수동 호출 시 이미지 리스트가 다시 로드된다`() = runTest {
        // Given
        coEvery { mockGetProfileImagesUseCase() } returns BaseResult.Success(sampleImageList)
        createViewModel()
        testDispatcher.scheduler.advanceUntilIdle()

        val newImageList = ProfileImageList(
