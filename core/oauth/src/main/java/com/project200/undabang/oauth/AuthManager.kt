    ...
    constructor(
        ...
        suspend fun logout(
            ...
            try {
                ...
                val manualLogoutUriBuilder =
                    serviceConfig.endSessionEndpoint!!.buildUpon()
                ...
                authService.performTokenRequest(tokenRefreshRequest) { tokenResponse, ex ->
                    ...
                    if (tokenResponse != null) {
                        ...
                        if (ex?.type == AuthorizationException.TYPE_OAUTH_TOKEN_ERROR && ex.error == "invalid_grant") {
                            Timber.tag(
                                TAG_DEBUG,
                            ).w("Refresh token is invalid (invalid_grant). Clearing local AuthState and forcing logout.")
