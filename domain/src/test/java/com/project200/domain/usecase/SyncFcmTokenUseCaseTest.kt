...
class SyncFcmTokenUseCaseTest {
    ...
    fun `로그인 전이면 전송하지 않고 SKIPPED를 반환한다`() = runTest {
        ...
        coVerify(exactly = 0) { mockFcmRepository.registerToken() }
    }
    ...
    fun `저장된 토큰이 없으면 전송하지 않고 SKIPPED를 반환한다`() = runTest {
        ...
        coEvery { mockFcmRepository.getSavedToken() } returns null
    ...
    fun `전송에 성공하면 SUCCESS를 반환한다`() = runTest {
        // Given
        coEvery { mockAuthRepository.getMemberId() } returns "member-1"
        coEvery { mockFcmRepository.getSavedToken() } returns "token-1"
        coEvery { mockFcmRepository.registerToken() } returns BaseResult.Success(Unit)
        ...
        coVerify(exactly = 1) { mockFcmRepository.registerToken() }
    }
    ...
    fun `전송에 실패하면 FAILURE를 반환한다`() = runTest {
        ...
        coEvery { mockAuthRepository.getMemberId() } returns "member-1"
        coEvery { mockFcmRepository.getSavedToken() } returns "token-1"
        coEvery { mockFcmRepository.registerToken() } returns BaseResult.Error("NETWORK_ERROR", "네트워크 오류")
    ...
    @Test
    fun `토큰이 없어 등록이 막히면 FAILURE를 반환한다`() = runTest {
        // Given: 조회 직후 토큰이 지워져 registerToken()이 NO_FCM_TOKEN으로 막힌 경우
        coEvery { mockAuthRepository.getMemberId() } returns "member-1"
        coEvery { mockFcmRepository.getSavedToken() } returns "token-1"
        coEvery { mockFcmRepository.registerToken() } returns
            BaseResult.Error("NO_FCM_TOKEN", "FCM 토큰이 없어 등록 요청을 보내지 않았습니다")
    ...
    fun `대기 중 토큰이 갱신되면 호출 시점의 값을 읽는다`() = runTest {
        ...
        coEvery { mockAuthRepository.getMemberId() } returns "member-1"
        coEvery { mockFcmRepository.getSavedToken() } returns "token-1" andThen "token-2"
        coEvery { mockFcmRepository.registerToken() } returns BaseResult.Success(Unit)
        ...
        // Then: 저장소를 매번 다시 읽는다
        assertThat(result).isEqualTo(FcmTokenSyncResult.SUCCESS)
        coVerify(exactly = 2) { mockFcmRepository.getSavedToken() }
    }
}
