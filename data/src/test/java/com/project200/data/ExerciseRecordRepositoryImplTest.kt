...
class ExerciseRecordRepositoryImplTest {
    ...
    private val sampleDtoWithoutPictures =
        GetExerciseRecordData(
    ...
    @Test
    fun `getExerciseDetail - API 성공 (succeed=true), 사진 있는 데이터`() =
        runTest(testIoDispatcher) {
            val recordId = 1L
            val apiResponse =
                BaseResponse(
                    succeed = true,
    ...
    @Test
    fun `getExerciseDetail - API 성공 (succeed=true), 사진 없는 데이터 (pictureDataList=null)`() =
        runTest(testIoDispatcher) {
            val recordId = 2L
            val apiResponse =
                BaseResponse(
    ...
    @Test
    fun `getExerciseDetail - 접근 권한 없음 (AUTHORIZATION_DENIED, HTTP 403)`() =
        runTest(testIoDispatcher) {
            val recordId = 10L
    ...
    @Test
    fun `getExerciseDetail - 잘못된 입력 값 (INVALID_INPUT_VALUE, HTTP 400)`() =
        runTest(testIoDispatcher) {
            val recordId = -1L
    ...
    @Test
    fun `getExerciseDetail - 운동 기록 없음 (EXERCISE_RECORD_NOT_FOUND, HTTP 404)`() =
        runTest(testIoDispatcher) {
            val recordId = 99999L
            val errorResponseObject =
                BaseResponse<Any?>( // data가 null
                    succeed = false,
